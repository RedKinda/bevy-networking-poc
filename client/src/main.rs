...
use common::bevy_networking_turbulence::{NetworkResource, NetworkingPlugin, NetworkEvent};
use common::events::*;
...
fn startup_test(mut commands: Commands, assets: Res<AssetServer>) {
    PlayerPointer::spawn(&mut commands, &0, &Default::default(), &NetworkSync { unique_id: 0 }, assets.load("player_pointer.png"));
}
...
fn startup(mut commands: Commands, mut net: ResMut<NetworkResource>) {
    ...
    let address = SocketAddr::new("192.168.0.102".parse().unwrap(), common::SERVER_PORT);
...
fn send_command(mut net: ResMut<NetworkResource>, command: PlayerCommand) {
    debug!("Sending command {}", common::serde_form::to_string(&command).unwrap());
    ...
    for event in reader.iter() {
        match event {
            ...
            NetworkEvent::Packet(handle, packet) => {
                debug!("Got a packet: {} fron handle {}", String::from_utf8_lossy(packet), handle)
            }
...
fn handle_movement_changes(mut events: EventReader<ServerEvent>, mut query: Query<(&NetworkSync, &mut Movable, &mut Transform)>) {
    for event in events.iter() {
        if let ServerEvent::EntityMovementChange(netsync, movable, pos) = event {
            if let Some((_, mut current_movable, mut current_transform)) = query.iter_mut().find(|unit| unit.0.unique_id == netsync.unique_id) {
...
fn capture_clicks(
    ...
    if mouse_input.just_pressed(MouseButton::Left) {
        ...
        if let Some(pointer) = my_pointer.iter().find(|(_, ctrl)| ctrl.owner == identity.player_id) {
            send_command(net, PlayerCommand::PointerMoveChange(
                *pointer.0,
                Movable::new(position)
            ))
        } else {
